...
function updateSlider(){
    ...
    //Add the 'previous ' cass to the image before the current one
    if (imageIndex -1 >=0) {
    ...
    //Add the 'next' class to the image before the current one
    ...
    //Add the 'inactive' cass to the other images
    images.forEach((image,index)=>{
        if(index !== imageIndex && index !== (imageIndex -1)%images.length && index !== (imageIndex +1) % images.length){
            image.classList.add('inactive');
        }
    });

    //set the opacity of all the background divs to -
    backgrounds.forEach((background) =>{
        background.style.opacity =0;
    });
    // If the current imge is active, set the opacity of the corresonding background div to 1
    if (images[imageIndex].classList.contains('active')) {
        backgrounds[imageIndex +1].style.opacity=1;
    }
