...
// =============== FOOTER ==========
document.getElementById('year').textContent = `@${new Date().getFullYear()} | Todos los derechos reservados`;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (aboutSection && cardsContainer && scrollImage && scrollImageText && sectionTitle) {
        document.addEventListener('scroll', () => {
            ...
            scrollImageText.style.opacity = scrollPosition > sectionTop + (sectionHeight * 1.15) ? '1' : '0';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateCarousel = () => {
        cards.forEach((card, index) => {
            ...
            card.classList.add(relativeIndex === 0 ? 'active' : relativeIndex === 1 ? 'right' : relativeIndex === cards.length - 1 ? 'left' : 'inactive');
        });
    };
